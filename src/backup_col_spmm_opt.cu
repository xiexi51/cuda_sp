...
__global__ void spmm_kernel_opt(int *ptr, int *idx, float *val, float *vin, float *vout, int num_v, int feat_in)
{
    ...
    for (int i = begin; i < end; i += BLOCK_Y)
    {
        ...
        mul_cache = v * __ldg(vin_offset + col);
// #pragma unroll
...
//         }
        for (int offset = 32/2; offset > 0; offset /= 2){
            mul_cache +=__shfl_down_sync(0xFFFFFFFF, mul_cache, offset,32);
        }
        // for (int offset=1; offset<32; offset*=2)
        //     mul_cache += __shfl_xor_sync(-1, mul_cache, offset);
    }
    vout[out_idx] = mul_cache;
}
